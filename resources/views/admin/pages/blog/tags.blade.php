...
@section('title', 'Tags - Car Image Editing')
            ...
            <div class="row align-items-end">
                <div class="col-sm mb-2 mb-sm-0">
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb breadcrumb-no-gutter">
                            ...
                            <li class="breadcrumb-item"><a class="breadcrumb-link" href="javascript:;">Tags</a></li>
                ...
                <div class="col-sm-auto">
                    <a class="btn btn-primary" href="javascript:;" data-toggle="modal" data-target="#addCollectionModal">
                        <i class="tio-user-add mr-1"></i> Add new tag
                    </a>
                </div>
            </div>
            <!-- End Row -->
                    ...
                    <div class="col-sm-6 col-md-4 mb-3 mb-sm-0">
                        <form>
                            ...
                            <div class="input-group input-group-merge input-group-flush">
                                ...
                                <input id="datatableSearch" type="search" class="form-control" placeholder="Search tags..." aria-label="Search users">
                            </div>
                            <!-- End Search -->
                        </form>
                    </div>
                    ...
                    @foreach($tags as $key => $tag)
                        <tr>
                            <td class="table-column-pr-0">
                                <div class="custom-control custom-checkbox">
                                    <input type="checkbox" class="custom-control-input check-collection" id="usersDataCheck{{$tag->id}}">
                                    <label class="custom-control-label" for="usersDataCheck{{$tag->id}}"></label>
                                </div>
                            </td>
                            <td class="table-column-pl-0">
                                <a class="d-flex align-items-center">
                                    <div class="avatar avatar-soft-dark avatar-circle">
                                        <span class="avatar-initials">{{ substr($tag->name, 0, 1) }}</span>
                                    </div>
                                    <div class="ml-3">
                                        <span class="d-block h5 text-hover-primary mb-0">{{ $tag->name }} <i class="tio-verified text-primary" data-toggle="tooltip" data-placement="top" title="Top endorsed"></i></span>
                                    </div>
                                </a>
                            </td>
                            <td>
                                <span class="d-block font-size-sm">/{{ $tag->slug }}</span>
                            </td>
                            <td>
                            ...
                            <td>
                                <div class="btn-group" role="group">
                                    <button class="btn btn-sm btn-white" type="button" data-toggle="modal" data-target="#editCollectionModal" onclick="editCollection({{ $tag->id }})">
                                    ...
                                    <div class="hs-unfold btn-group">
                                        <a class="js-hs-unfold-invoker btn btn-icon btn-sm btn-white dropdown-toggle dropdown-toggle-empty" href="javascript:;"
                                           data-hs-unfold-options='{
                                             "target": "#productsEditDropdown{{$tag->id}}",
                                             "type": "css-animation",
                                             "smartPositionOffEl": "#datatable"
                                           }'></a>

                                        <div id="productsEditDropdown{{$tag->id}}" class="hs-unfold-content dropdown-unfold dropdown-menu dropdown-menu-right mt-1">
                    ...
                    <div class="col-sm mb-2 mb-sm-0">
                        <div class="d-flex justify-content-center justify-content-sm-start align-items-center">
                            ...
                            <select id="datatableEntries" class="js-select2-custom"
                                ...
                                @if (count($tags) > 5)
                                    @for($i = 5; $i <= count($tags); $i += 5)
                                         <option value="{{ $i }}" @if($i === 10) selected @endif>{{ $i }}</option>
                                    @endfor
                                @else
                                    <option value="{{ count($tags) }}" selected>{{ count($tags) }}</option>
                                @endif
                            </select>
                            <!-- End Select -->
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
            <div class="modal-content">
                ...
                <div class="modal-header">
                    <h4 id="editUserModalTitle" class="modal-title">Add New Tag</h4>
                    ...
                    <div class="collection-form">
                        <form action="{{ route('tags.store') }}" method="POST">
                            ...
                            <div class="row form-group">
                                <label for="add_name" class="col-sm-3 col-form-label input-label">Tag name</label>
    ...
    <div class="modal fade" id="editCollectionModal" tabindex="-1" role="dialog" aria-labelledby="editCollectionModalTitle" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
            <div class="modal-content">
                <!-- Header -->
                <div class="modal-header">
                    <h4 id="editUserModalTitle" class="modal-title">Edit Tag</h4>
                ...
                <div class="modal-body">
                    <div class="collection-form">
                        <form action="{{ route('tags.update', 'edit') }}" method="POST">
                        @method('PUT')
                        @csrf
                            <input type="hidden" name="id" id="edit_id">
                            <!-- Form Group -->
                            <div class="row form-group">
                                <label for="edit_name" class="col-sm-3 col-form-label input-label">Tag name</label>
...
@section('script')
    <script>
        ...
        async function editCollection(id) {
            try {
                const response = await fetch(`/admin/blog/tags/${id}/edit`);
                const tag = await response.json();
                if (tag && response.status === 200) {
                    ...
                    document.getElementById('edit_meta_title').value = tag.meta_title;
                    document.getElementById('edit_meta_description').value = tag.meta_description;
                    document.getElementById('edit_meta_description').value = tag.meta_description;
                    if (tag.meta_keywords) {
                        let keywords = JSON.parse(tag.meta_keywords);
